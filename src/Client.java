...
public class Client implements Runnable{
    ...
    public static void main(String args[]) throws IOException, ArrayIndexOutOfBoundsException {
        ...
        try {
            ...
            client.connect("localhost", 600);
            client.run();
        } catch (IOException | UnresolvedAddressException | TransformerException | ParserConfigurationException e) {
            ClientInterface userInterface = new ClientInterface(new InputStreamReader(System.in, StandardCharsets.UTF_8), new OutputStreamWriter(System.out, StandardCharsets.UTF_8), true);
            if (userInterface.readWithMessage("Хотите попробывать снова подключиться? Введите: Да/Нет.", false).equals("Да")){
                try {
                    Client client = new Client();
                    client.connect("localhost", 600);
                    client.run();
                } catch (IOException | UnresolvedAddressException | TransformerException | ParserConfigurationException ioException) {
                    System.exit(-1);
                }
            } else {System.exit(-1);}
        }
